...
namespace sst_core.Core
{
    public class CountryCore : ICountryCore
    {
        ...
        private readonly ILogger<CountryCore> _logger;

        public CountryCore(
            ILogger<CountryCore> logger,
        ...
        public async Task<GeneralResponse> GetAll()
        {
            ...
            try
            {
                var users = new List<CountryResponse>();

                var userDb = await _companyData.GetAll();

                userDb?.All(x =>
                {
                    users.Add(new CountryResponse(x));
                    return true;
                });
